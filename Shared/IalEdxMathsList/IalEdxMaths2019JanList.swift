...
struct IalEdxMaths2019JanView1: View {
...
struct IalEdxMaths2019JanList3: View {
    var  body: some View {
        List {
            ForEach(IalMaths2019JanData3)  { ialMaths2019Jan3 in
                NavigationLink(destination: IalEdxMaths2019JanWebView3(ialMaths2019Jan3: ialMaths2019Jan3)) {
                    Text(ialMaths2019Jan3.name)
                }
            }
        }
    }
...
struct IalEdxMaths2019JanWebView3: View {
    @Environment(\.horizontalSizeClass) var horizontalSizeClass
    @Environment(\.verticalSizeClass) var verticalSizeClass
    @State private var isPresented = false
    @State private var isActivityPopoverPresented = false
    @State private var isActivitySheetPresented = false
    var ialMaths2019Jan3: IalMaths2019Jan3
    
    var body: some View {
        Webview(url: ialMaths2019Jan3.url)
            .edgesIgnoringSafeArea(.all)
            .navigationBarTitle(ialMaths2019Jan3.name, displayMode: .inline)
            .navigationBarItems(trailing: shareButton)
            .sheet(isPresented: $isActivitySheetPresented, content: activityView)
    ...
    private func activityView() -> some View {
        let url = URL(string: ialMaths2019Jan3.url)!
        let filename = url.pathComponents.last!
        let fileManager = FileManager.default
        let itemURL = fileManager.temporaryDirectory.appendingPathComponent(filename)
        let data: Data
...
struct IalMaths2019Jan3: Hashable, Codable, Identifiable {
    var id: Int
    var name: String
    var url: String
}
...
let IalMaths2019JanData3: [IalMaths2019Jan3] = load("IalMaths2019Jan3.json")
