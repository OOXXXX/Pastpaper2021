...
struct IalEdxMaths2018OctView1: View {
...
struct IalEdxMaths2018OctList3: View {
    var  body: some View {
        List {
            ForEach(IalMaths2018OctData3)  { ialMaths2018Oct3 in
                NavigationLink(destination: IalEdxMaths2018OctWebView3(ialMaths2018Oct3: ialMaths2018Oct3)) {
                    Text(ialMaths2018Oct3.name)
                }
            }
        }
    }
...
struct IalEdxMaths2018OctWebView3: View {
    @Environment(\.horizontalSizeClass) var horizontalSizeClass
    @Environment(\.verticalSizeClass) var verticalSizeClass
    @State private var isPresented = false
    @State private var isActivityPopoverPresented = false
    @State private var isActivitySheetPresented = false
    var ialMaths2018Oct3: IalMaths2018Oct3
    
    var body: some View {
        Webview(url: ialMaths2018Oct3.url)
            .edgesIgnoringSafeArea(.all)
            .navigationBarTitle(ialMaths2018Oct3.name, displayMode: .inline)
            .navigationBarItems(trailing: shareButton)
            .sheet(isPresented: $isActivitySheetPresented, content: activityView)
    ...
    private func activityView() -> some View {
        let url = URL(string: ialMaths2018Oct3.url)!
        let filename = url.pathComponents.last!
        let fileManager = FileManager.default
        let itemURL = fileManager.temporaryDirectory.appendingPathComponent(filename)
        let data: Data
...
struct IalMaths2018Oct3: Hashable, Codable, Identifiable {
    var id: Int
    var name: String
    var url: String
}
...
let IalMaths2018OctData3: [IalMaths2018Oct3] = load("IalMaths2018Oct3.json")
